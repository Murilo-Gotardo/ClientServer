...
use crate::json_sender::{JsonSender, LocaleEnum};
...
pub fn set(requisition: Requisition, socket: &UdpSocket, addr: SocketAddr) {
    ...
    match find_locale_on_json(requisition, &mut locales, "set".to_string()) {
        Ok(response_locale) => {
            let json = JsonSender::make_response_json(LocaleEnum::Locale(response_locale));
...
pub fn get_all(socket: &UdpSocket, addr: SocketAddr) {
    let mut file = File::open("src/local.json").unwrap();
    let mut data = String::new();
    file.read_to_string(&mut data).unwrap();

    let locales: Locales = serde_json::from_str(&data).expect("JSON inválido");

    let json = JsonSender::make_response_json(LocaleEnum::Locales(locales));
    JsonSender::send_json_to_client(json, socket, addr);
}
    ...
    match mode.as_str() { 
        "set" => {
            for locale in &mut locales.locale_list {
                if locale.locate == requisition.locate().clone().unwrap() {
        ...
        "get" => {
            return Ok(locales.locale_list.clone()
                .into_iter()
                .find(|l| l.locate == requisition.locate().clone().unwrap())
                .unwrap());
        },
        _ => {}
    }
